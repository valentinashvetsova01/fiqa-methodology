        ...
        const MethodologyDiagram = () => {
            ...
            const pipelineDetails = {
                advice: {
                    ...
                    steps: [
                        ...
                        { name: "Cross-Encoder Re-rank", tech: "BGE-reranker-base", detail: "Final = λ₁·CE + λ₂·ArgQuality + λ₃·DiscourseScore" }
                    ]
                },
                comparison: {
                ...
                explanation: {
                    ...
                    color: "#ec4899",
                    steps: [
                        { name: "HyDE Enhancement", tech: "LLM generation", detail: "Generate hypothetical answer for query expansion" },
                        { name: "Causal Patterns", tech: "Discourse markers", detail: "'because', 'due to', 'caused by', 'leads to'" },
                        { name: "Reasoning Chains", tech: "Chain detection", detail: "Multi-step reasoning: A → B → C" }
                    ]
                }
            };
            ...
            const PipelineCard = ({ id, title, emoji, color, isSelected }) => (
                <div 
                    ...
                    className={`cursor-pointer p-4 rounded-lg border-2 transition-all duration-300 ${isSelected ? 'shadow-lg scale-105' : 'hover:shadow-md'}`}
                    style={{ borderColor: color, backgroundColor: isSelected ? `${color}20` : 'white' }}
                >
                    <div className="text-center font-bold" style={{ color }}>{emoji} {title}</div>
                </div>
            );
            ...
            return (
                <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
                    <div className="max-w-5xl mx-auto">
                        ...
                        <p className="text-center text-slate-500 mb-8">Method Overview — Click pipelines for details</p>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl p-8">
                            ...
                            <Box className="bg-amber-50 border-2 border-amber-400 max-w-lg mx-auto text-center">
                                <div className="text-sm font-semibold text-amber-700">⚡ UNIVERSAL QUERY EXPANSION</div>
                            ...
                            <Box className="bg-indigo-50 border-2 border-indigo-400 max-w-lg mx-auto text-center">
                                <div className="text-sm font-semibold text-indigo-700">🔍 FIRST-STAGE RETRIEVAL (shared)</div>
                                <div className="text-slate-600 text-xs mt-1">BM25 + Dense → RRF → Top-50 candidates</div>
                            </Box>

                            <Arrow />

                            <Box className="bg-orange-50 border-2 border-orange-500 max-w-md mx-auto text-center">
                                <div className="text-sm font-semibold text-orange-700">🎯 INTENT CLASSIFIER</div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                                <PipelineCard id="advice" title="ADVICE" emoji="💡" color="#22c55e" isSelected={selectedPipeline === 'advice'} />
                                <PipelineCard id="comparison" title="COMPARISON" emoji="⚖️" color="#f59e0b" isSelected={selectedPipeline === 'comparison'} />
                                <PipelineCard id="factual" title="FACTUAL" emoji="📖" color="#3b82f6" isSelected={selectedPipeline === 'factual'} />
                                <PipelineCard id="explanation" title="EXPLANATION" emoji="❓" color="#ec4899" isSelected={selectedPipeline === 'explanation'} />
                            </div>
                            ...
                            <Box className="bg-indigo-50 border-2 border-indigo-400 max-w-md mx-auto text-center">
                                <div className="text-sm font-semibold text-indigo-700">📤 RANKED RESULTS</div>
